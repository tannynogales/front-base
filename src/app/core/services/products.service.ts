...
export class ProductsService {
  ...
  fetch(categorySlug: string, page: number = 1) {
    ...
    const url =
      this.baseUrl +
      `/products?${environment.productStateFilters}&filters[categories][slug][$eq]=${categorySlug}&[categories][site][id][$eq]=${this.siteID}&populate[primary_image]=*${this._filterByPrice}&populate[categories]=*&populate[product_attribute_values]=*&pagination[pageSize]=12&pagination[page]=${page}&sort=price:${this.order}`;
    // console.log(url);
    this.httpClient
      .get<Response>(url)
      .pipe(
